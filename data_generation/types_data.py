...
def get_type_data(type_id, directory):

    data = utils.create_directory_and_return_data(directory, type_id)
    name = data["name"]

    return (name,)


def insert_types(cur, script, upper_types_limit, types_directory_string):
    ...
    for i in range(1, upper_types_limit):  
        try:
            data = get_type_data(i, types_directory_string)
    ...
    try: 
        mystery = ('???',)
        cur.execute("INSERT INTO pokemon.type (name)\nVALUES (%s)", mystery)
        print(f"✅ Inserting special type {mystery[0]}")
        ...
        shadow = ('shadow',)
        cur.execute("INSERT INTO pokemon.type (name)\nVALUES (%s)", shadow)
        print(f"✅ Inserting special type {shadow[0]}")
