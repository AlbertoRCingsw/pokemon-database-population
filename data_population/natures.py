...
def process_and_insert_natures(cursor, natures, natures_directory, entity_name):
    ...
    for nature_index in natures:
        ...
        try:
            db.simple_insert(cursor, "nature", "(name, neutral, stat_up, stat_down)", nature_tuple)
        except Exception:
            logger.error(f"Error inserting {name} Nature", exc_info=True)
            raise

def insert_natures(cursor, natures_directory):
    logger.info("Entering insert_natures")
    
    entity_name = "nature"
    natures = utils.get_entity_data(entity_name)

    process_and_insert_natures(cursor, natures, natures_directory, entity_name)

    logger.info("Exiting insert_natures")
